...
// region Codec
export const BLUEPRINT_CODEC = registerDeletableHandlerPatch({
	id: 'animated_java:codec/blueprint',
	create() {
		return new Blockbench.Codec(`animated_java:codec/blueprint`, {
			name: 'Blueprint',
			...
			compile(options = {}) {
				...
				if (Blockbench.AnimationController.all.length > 0) {
					...
					for (const controller of Blockbench.AnimationController.all) {
						...
						model.animation_controllers.push(
							controller.getUndoCopy(animationOptions, true)
						)
					}
				}
			...
			export() {
				...
				let exportName = Project.name
				// eslint-disable-next-line @typescript-eslint/prefer-nullish-coalescing
				if (!exportName) exportName = 'unnamed'

				Blockbench.export({
					resource_id: 'animated_java_blueprint.export',
					name: exportName + '.ajblueprint',
					...
					custom_writer: (content: string, path: string) => {
						...
						if (existsSync(PathModule.dirname(path))) {
							...
							Blockbench.showMessageBox({
								...
								message: translate(
									'error.blueprint_export_path_doesnt_exist',
									path
								),
							})
						}
